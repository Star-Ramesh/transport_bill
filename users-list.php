...
if (isset($_GET['action']) && $_GET['action'] === 'toggle_active') {
    ...
    $check = mysqli_query(
        $conn,
        "SELECT id, username, active FROM `user` WHERE id = $id LIMIT 1"
    );
    ...
    $upd = mysqli_query($conn, "UPDATE `user` SET active = $new WHERE id = $id");
...
/* =========================================================
   PAGE DATA
   ========================================================= */
$pageTitle = 'Users | Billing Portal';

$sql = "SELECT u.id, u.username, u.full_name, u.branch_id, u.role_id, u.active, u.created_at,
                                            ...
                                            <tr
                                                ...
                                                <td class="align-middle">
                                                    <span class="user-name">
                                                        <?= htmlspecialchars($row['full_name'] ?: $row['username'], ENT_QUOTES, 'UTF-8') ?>
                                                    </span>
                                                    <?php if ($isSelf): ?>
                                                ...
                                                <td class="align-middle text-center text-nowrap">
                                                    <div class="table-actions">
                                                        ...
                                                        <?php if (hasPermission('user.delete') && !$isSelf): ?>
    ...
    <script>
        $(function() {
            ...
            $(document).on('change', '.js-toggle-active', function() {
                ...
                $.ajax({
                        url: 'users-list.php?action=toggle_active',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            id: id
