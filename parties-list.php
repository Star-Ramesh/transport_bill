...
if (isset($_GET['action']) && $_GET['action'] === 'toggle_active') {
    ...
    $upd = mysqli_query($conn, "UPDATE party SET active = $new WHERE id = $id");
...
<head>
    ...
    <style>
        ...
        #partiesTable tbody tr:hover>td {
                                            ...
                                            <tr
                                                ...
                                                <td class="align-middle">
                                                    <span class="party-name">
                                                        <?= htmlspecialchars($row['legal_name'], ENT_QUOTES, 'UTF-8') ?>
                                                    </span>
                                                    <?php if (!empty($row['trade_name'])): ?>
                                                        <br>
                                                        <span class="party-trade">
                                                            <?= htmlspecialchars($row['trade_name'], ENT_QUOTES, 'UTF-8') ?>
                                                        </span>
                                                    <?php endif; ?>
                                                </td>
                                                ...
                                                <td class="align-middle">
                                                    <?php if (!empty($row['gstin'])): ?>
                                                        <span class="gstin-code">
                                                            <?= htmlspecialchars($row['gstin'], ENT_QUOTES, 'UTF-8') ?>
                                                        </span>
                                                    <?php else: ?>
                                                ...
                                                <td class="align-middle text-center text-nowrap">
                                                    <div class="table-actions">
                                                        ...
                                                        <?php if (hasPermission('party.edit')): ?>
                                                            <a href="party.php?id=<?= (int) $row['id'] ?>"
                                                        ...
                                                        <?php if (hasPermission('party.delete')): ?>
        ...
        $(function() {
            ...
            var table = $('#partiesTable').DataTable({
                ...
                language: {
                    ...
                    infoEmpty: 'No parties',
                    infoFiltered: '(filtered from _MAX_)',
                    zeroRecords: 'No matching parties found',
                    paginate: {
            ...
            $(document).on('change', '.js-toggle-active', function() {
                ...
                $.ajax({
                        url: 'parties-list.php?action=toggle_active',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            id: id
